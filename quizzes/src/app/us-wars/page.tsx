...
import { US_WARS, matchUSWar } from '@/lib/us-wars';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = US_WARS.length;

export default function USWarsQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('us-wars-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('us-wars-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('us-wars-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('us-wars-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('us-wars-guessed');
      localStorage.removeItem('us-wars-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">US Wars</h1>
      <p className="text-gray-500 text-sm mb-4">Name {TOTAL} major US wars — ranked by start year</p>
          ...
          {US_WARS.map(({ name, years }, i) => {
            ...
            return (
              <tr key={name}>
                ...
                <td style={{ border: '1px solid black', padding: '4px 8px', background: '#f9fafb', color: '#6b7280', width: '90px', fontSize: '12px' }}>
                  {years}
                </td>
                <td style={{
