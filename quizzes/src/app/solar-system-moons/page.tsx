...
import { MOONS, MOON_SECTIONS, matchMoon } from '@/lib/solar-system-moons';
...
type TableRow =
  ...
  | { type: 'moon'; name: string; index: number };

const TABLE_ROWS: TableRow[] = [];
let _idx = 0;
MOON_SECTIONS.forEach(s => {
  TABLE_ROWS.push({ type: 'header', text: s.header });
  s.moons.forEach(name => TABLE_ROWS.push({ type: 'moon', name, index: _idx++ }));
});

export default function SolarSystemMoonsQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('solar-moons-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('solar-moons-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('solar-moons-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('solar-moons-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('solar-moons-guessed');
      localStorage.removeItem('solar-moons-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Solar System Moons</h1>
      <p className="text-gray-500 text-sm mb-4">Name {TOTAL} major moons — grouped by planet</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} moons!</p>}
