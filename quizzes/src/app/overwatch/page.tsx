...
import { OW_HEROES, OW_SECTIONS, matchOWHero } from '@/lib/overwatch';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = OW_HEROES.length;

type TableRow =
  | { type: 'header'; text: string }
  | { type: 'hero'; name: string; index: number };

const TABLE_ROWS: TableRow[] = [];
let _idx = 0;
OW_SECTIONS.forEach(s => {
  TABLE_ROWS.push({ type: 'header', text: s.header });
  s.heroes.forEach(name => TABLE_ROWS.push({ type: 'hero', name, index: _idx++ }));
});

export default function OverwatchQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('overwatch-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('overwatch-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('overwatch-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('overwatch-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('overwatch-guessed');
      localStorage.removeItem('overwatch-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Overwatch 2 Heroes</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} heroes — nicknames accepted</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} heroes!</p>}
