...
export default function CurrenciesQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('currencies-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('currencies-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('currencies-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('currencies-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!val) return;
    const matched = matchCurrency(val);
      ...
      {gameState === 'playing' && (
        <form onSubmit={handleSubmit} className="mb-4">
          <input
            ...
            placeholder="Type a currency name or code..."
            autoComplete="off"
            autoCorrect="off"
            spellCheck={false}
