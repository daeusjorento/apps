...
import { SCHOOLS, SCHOOL_SECTIONS, matchSchool } from '@/lib/elite-schools';
...
type TableRow =
  ...
  | { type: 'school'; name: string; index: number };

const TABLE_ROWS: TableRow[] = [];
let _idx = 0;
SCHOOL_SECTIONS.forEach(s => {
  TABLE_ROWS.push({ type: 'header', text: s.header });
  s.schools.forEach(name => TABLE_ROWS.push({ type: 'school', name, index: _idx++ }));
});

export default function EliteSchoolsQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('elite-schools-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('elite-schools-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('elite-schools-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('elite-schools-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('elite-schools-guessed');
      localStorage.removeItem('elite-schools-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Ivy League &amp; NESCAC Schools</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} schools — short names accepted</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} schools!</p>}
