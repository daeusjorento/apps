...
export default function NBATeamsQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem('nba-teams-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('nba-teams-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('nba-teams-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('nba-teams-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!val) return;
    const matched = matchNBATeam(val);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('nba-teams-guessed');
      localStorage.removeItem('nba-teams-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">NBA Teams</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} NBA teams — city, nickname, or both accepted</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} NBA teams!</p>}
