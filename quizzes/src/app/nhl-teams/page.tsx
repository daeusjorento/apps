...
import { NHL_TEAMS, NHL_SECTIONS, matchNHLTeam } from '@/lib/nhl-teams';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = NHL_TEAMS.length;
...
export default function NHLTeamsQuiz() {
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('nhl-teams-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('nhl-teams-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('nhl-teams-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!val) return;
    const matched = matchNHLTeam(val);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('nhl-teams-guessed');
      localStorage.removeItem('nhl-teams-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">NHL Teams</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} NHL teams — city, nickname, or both accepted</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} NHL teams!</p>}
