...
import { SINS, matchSin } from '@/lib/seven-deadly-sins';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = SINS.length;

export default function SevenDeadlySinsQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('seven-deadly-sins-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('seven-deadly-sins-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('seven-deadly-sins-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('seven-deadly-sins-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('seven-deadly-sins-guessed');
      localStorage.removeItem('seven-deadly-sins-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Seven Deadly Sins</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} deadly sins — Latin names also accepted</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} deadly sins!</p>}
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%' }}>
        <tbody>
          {SINS.map((sin, i) => {
            const isGuessed = guessed.has(sin);
