...
import { DISNEY_MOVIES, DISNEY_SECTIONS, matchDisneyMovie } from '@/lib/disney-movies';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = DISNEY_MOVIES.length;

type TableRow =
  | { type: 'header'; text: string }
  | { type: 'movie'; name: string; index: number };

const TABLE_ROWS: TableRow[] = [];
let _idx = 0;
DISNEY_SECTIONS.forEach(s => {
  TABLE_ROWS.push({ type: 'header', text: s.header });
  s.movies.forEach(name => TABLE_ROWS.push({ type: 'movie', name, index: _idx++ }));
});

export default function DisneyMoviesQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('disney-movies-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('disney-movies-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('disney-movies-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('disney-movies-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!val) return;
    const matched = matchDisneyMovie(val);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('disney-movies-guessed');
      localStorage.removeItem('disney-movies-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Disney Animated Movies</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} classic and modern Disney animated films</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} Disney animated movies!</p>}
