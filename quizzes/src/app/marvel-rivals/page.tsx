...
import { MR_CHARACTERS, MR_SECTIONS, matchMRCharacter } from '@/lib/marvel-rivals';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = MR_CHARACTERS.length;
...
export default function MarvelRivalsQuiz() {
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('marvel-rivals-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('marvel-rivals-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('marvel-rivals-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!val) return;
    const matched = matchMRCharacter(val);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('marvel-rivals-guessed');
      localStorage.removeItem('marvel-rivals-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Marvel Rivals Characters</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} characters — real names and nicknames accepted</p>
