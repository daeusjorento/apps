...
export default function CanadaProvincesQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem('canada-provinces-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('canada-provinces-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('canada-provinces-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('canada-provinces-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('canada-provinces-guessed');
      localStorage.removeItem('canada-provinces-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Canadian Provinces &amp; Territories</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} provinces and territories — abbreviations accepted</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} provinces and territories!</p>}
