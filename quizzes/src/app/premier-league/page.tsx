...
import { PREMIER_LEAGUE_TEAMS, matchPLTeam } from '@/lib/premier-league';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = PREMIER_LEAGUE_TEAMS.length;

export default function PremierLeagueQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('premier-league-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('premier-league-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('premier-league-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('premier-league-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('premier-league-guessed');
      localStorage.removeItem('premier-league-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Premier League Teams</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} Premier League teams (2024–25) — club name or nickname accepted</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} Premier League teams!</p>}
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%' }}>
        <tbody>
          {PREMIER_LEAGUE_TEAMS.map((name, i) => {
