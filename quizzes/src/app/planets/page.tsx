...
import { PLANETS, PLANET_SECTIONS, matchPlanet } from '@/lib/planets';
...
type TableRow =
  ...
  | { type: 'planet'; name: string; index: number };

const TABLE_ROWS: TableRow[] = [];
let _idx = 0;
PLANET_SECTIONS.forEach(s => {
  TABLE_ROWS.push({ type: 'header', text: s.header });
  s.planets.forEach(name => TABLE_ROWS.push({ type: 'planet', name, index: _idx++ }));
});

export default function PlanetsQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('planets-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('planets-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('planets-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('planets-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('planets-guessed');
      localStorage.removeItem('planets-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Planets of the Solar System</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} planets in order from the Sun</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} planets!</p>}
