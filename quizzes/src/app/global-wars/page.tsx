...
import { GLOBAL_WARS, matchGlobalWar } from '@/lib/global-wars';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = GLOBAL_WARS.length;

export default function GlobalWarsQuiz() {
  const [guessed, setGuessed] = useState<Set<string>>(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const saved = localStorage.getItem('global-wars-guessed');
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('global-wars-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('global-wars-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('global-wars-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!val) return;
    const matched = matchGlobalWar(val);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('global-wars-guessed');
      localStorage.removeItem('global-wars-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Deadliest Wars in History</h1>
      <p className="text-gray-500 text-sm mb-4">Name {TOTAL} wars ranked by estimated casualties — estimates vary widely by source</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} wars!</p>}
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%' }}>
        <tbody>
          {GLOBAL_WARS.map(({ name, casualties }, i) => {
            ...
            return (
              <tr key={name}>
                ...
                <td style={{ border: '1px solid black', padding: '4px 8px', background: '#f9fafb', color: '#6b7280', width: '60px', fontSize: '12px', textAlign: 'right' }}>
                  {casualties}
                </td>
                <td style={{
