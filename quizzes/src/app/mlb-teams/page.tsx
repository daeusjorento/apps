...
import { MLB_TEAMS, MLB_SECTIONS, matchMLBTeam } from '@/lib/mlb-teams';

type GameState = 'playing' | 'given-up' | 'complete';
const TOTAL = MLB_TEAMS.length;
...
export default function MLBTeamsQuiz() {
  ...
  const [gameState, setGameState] = useState<GameState>(() => {
    ...
    try {
      return (localStorage.getItem('mlb-teams-gamestate') as GameState) || 'playing';
  ...
  useEffect(() => {
    try { localStorage.setItem('mlb-teams-guessed', JSON.stringify(Array.from(guessed))); }
    catch { /* ignore */ }
  }, [guessed]);

  useEffect(() => {
    try { localStorage.setItem('mlb-teams-gamestate', gameState); }
    catch { /* ignore */ }
  }, [gameState]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!val) return;
    const matched = matchMLBTeam(val);
  ...
  const handleReset = () => {
    try {
      localStorage.removeItem('mlb-teams-guessed');
      localStorage.removeItem('mlb-teams-gamestate');
    } catch { /* ignore */ }
    setGuessed(new Set());
    setInput('');
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-8">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">MLB Teams</h1>
      <p className="text-gray-500 text-sm mb-4">Name all {TOTAL} MLB teams — city, nickname, or both accepted</p>
      ...
      {gameState === 'complete' && <p className="text-green-600 font-semibold mb-4">You named all {TOTAL} MLB teams!</p>}
